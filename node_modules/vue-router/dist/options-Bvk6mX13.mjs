...
const diagnostics = /*#__PURE__*/ defineDiagnostics({
	...
	codes: {
		...
		VUE_ROUTER_B0020: {
			why: (p) => `duplicate definePage() call in file "${p.filename}". Only the first one is used, the rest are ignored.`,
		...
		VUE_ROUTER_B0021: {
			why: (p) => `param "${p.paramName}" declares a parser twice in "${p.segment}": "${p.filenameParser}" in the file name and "${p.declaredParser}" in \`params.path\`. Keeping "${p.declaredParser}".`,
...
//#endregion
export { formatMultilineUnion as _, resolveOverridableOption as a, toStringLiteral as b, ImportsMap as c, getFileBasedRouteName as d, getPascalCaseRouteName as f, throttle as g, mergeRouteRecordOverride as h, resolveOptions as i, appendExtensionListToPattern as l, logTree as m, DEFAULT_PARAM_PARSERS_OPTIONS as n, diagnostics as o, joinPath as p, mergeAllExtensions as r, ESCAPED_TRAILING_SLASH_RE as s, DEFAULT_OPTIONS as t, asRoutePath as u, isAbsolutePath as v, ts as x, pad as y };

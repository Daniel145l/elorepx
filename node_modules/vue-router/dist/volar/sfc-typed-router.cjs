...
VLS_CTX: /\b__VLS_ctx.\$route\b/g } };
	return {
		...
		resolveEmbeddedCode(fileName, sfc, embeddedCode) {
			...
			function visit(node) {
				if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) && ts.idText(node.expression) === "useRoute" && !node.typeArguments && !node.arguments.length) {
					if (!sfc.scriptSetup.lang.startsWith("js")) (0, muggle_string.replaceSourceRange)(embeddedCode.content, sfc.scriptSetup.name, node.expression.end, node.expression.end, useRouteNameTypeParam);
					else {
				...
				} else if (ts.isTypeQueryNode(node) && ts.isIdentifier(node.exprName) && ts.idText(node.exprName) === "useRoute" && !node.typeArguments && !sfc.scriptSetup.lang.startsWith("js")) (0, muggle_string.replaceSourceRange)(embeddedCode.content, sfc.scriptSetup.name, node.exprName.end, node.exprName.end, useRouteNameTypeParam);
